...
def generate_plots(N, mu, sigma2, S):
    ...
    X = X.reshape(-1, 1)  # Replace with code to fit the model
    model.fit(X, Y)
    ...
    plt.plot(X, slope * X + intercept, color='red', label=f'Fitted Line: y = {intercept:.2f} + {slope:.2f}x')  # Regression line
    plt.xlabel("X")
    plt.ylabel("Y")
    ...
    for _ in range(S):
        ...
        sim_model.fit(X_sim_reshaped, Y_sim) # Replace with code to fit model
